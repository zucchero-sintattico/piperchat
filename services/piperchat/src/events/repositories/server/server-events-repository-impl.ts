...
export class ServerEventsRepositoryImpl implements ServerEventsRepository {
  ...
  async publishServerCreated(payload: any) {
    ...
    if (channel) {
      channel.publish('servers', 'server.created', Buffer.from(JSON.stringify(payload)))
    }
  }
  ...
  async publishServerUpdated(payload: any) {
    const channel = this.getChannel()
    if (channel) {
      channel.publish('servers', 'server.updated', Buffer.from(JSON.stringify(payload)))
    }
  }
  ...
  async publishServerDeleted(payload: any) {
    const channel = this.getChannel()
    if (channel) {
      channel.publish('servers', 'server.deleted', Buffer.from(JSON.stringify(payload)))
    }
  }
  ...
  async publishUserLeft(payload: any) {
    const channel = this.getChannel()
    if (channel) {
      channel.publish('servers', 'server.user.left', Buffer.from(JSON.stringify(payload)))
    }
  }
