    ...
    <div class="column">
      <div class="col">
        ...
        <div class="q-ma-md" style="text-align: center">
          <q-btn color="primary" label="Create channel" icon="add" style="justify-content: space-between"
            @click="isNewChannelFormActive = true" />
        </div>
        <q-dialog v-model="isNewChannelFormActive" persistent transition-show="scale" transition-hide="scale">
          <NewChannelForm @close="isNewChannelFormActive = false" />
        </q-dialog>
        <!-- end Create new server -->

        <!-- start Message channels -->
        <q-list bordered separator class="text-white text-h5" v-for="channel in selectedServer?.channels?.filter(
